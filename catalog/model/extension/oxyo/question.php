...
class ModelExtensionOxyoQuestion extends Model
{
    ...
    public function addQuestion($product_id, $data)
    {
        ...
        $subject = sprintf($this->language->get('oxyo_text_subject'), $this->config->get('config_name'));

        $message  = $this->language->get('oxyo_text_waiting') . "\n";
        $message .= sprintf($this->language->get('oxyo_text_product_qa'), $this->db->escape(strip_tags($product_info['name']))) . "\n";
        $message .= sprintf($this->language->get('oxyo_text_author'), $this->db->escape(strip_tags($data['name']))) . " (" . $this->db->escape(strip_tags($data['email'])) . ")" . "\n";
        $message .= $this->language->get('oxyo_text_question') . "\n";
