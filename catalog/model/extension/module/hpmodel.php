...
class ModelExtensionModuleHpmodel extends Model
{
    ...
    public function getProducts($product_ids, $limit = 0)
    {
        ...
        $query = $this->db->query("SELECT p.*, pd.*, m.name as manufacturer, p.manufacturer_id as manufacturer_id, p.image AS product_image, IF(hl.image = '', p.image, hl.image) AS image, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status,  (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON p.product_id = pd.product_id LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) LEFT JOIN " . DB_PREFIX . "hpmodel_links hl ON (p.product_id = hl.product_id) WHERE p.product_id IN (" . implode(',', $filtered_product_ids) . ") AND p.status = 1 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY hl.sort ASC, IF(hl.parent_id = hl.product_id, 0, 1), p.price " . ($limit > 0 ? " LIMIT " . (int)$limit : ''));
    ...
    public function getAttributeValueSortOrder($attribute_id, $value)
    {
