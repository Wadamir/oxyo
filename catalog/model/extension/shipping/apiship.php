...
class ModelExtensionShippingApiship extends Model
{
    ...
    {
        if ($provider == '')
            return '<img class="apiship_img_providers" src="' . HTTP_SERVER . 'catalog/view/theme/default/image/shipping/apiship_map.png">';
        else
    ...
    public function get_quote_list($address, $full_list = false)
    {
        ...
        if ($full_list == false) {
            ...
            if (isset($data['deliveryToPoint'])) $providers = $data['deliveryToPoint'];
            else $providers = [];
            foreach ($providers as $provider) {
                if (isset($provider['tariffs'])) $tariffs = $provider['tariffs'];
                else $tariffs = [];
...
*/
        ...
        if (isset($data['deliveryToDoor'])) $providers = $data['deliveryToDoor'];
        else $providers = [];
        foreach ($providers as $provider) {
            if (isset($provider['tariffs'])) $tariffs = $provider['tariffs'];
            else $tariffs = [];
        ...
        if ($this->apiship_params['shipping_apiship_error_stub_show']) {
            if (empty($quote_data)) {
                ...
                if (isset($data['message'])) {
                    ...
                    $title = $data['message'];
                    if (isset($data['errors'])) $errors = $data['errors'];
                    else $errors = [];
        ...
        $apiship_script = <<<EOT
				...
				<script type="text/javascript">
					...
					function create_shipping_method_template(data) {
					  const apishipImage = data.apiship_image 
					    ? `<img src="\${data.apiship_image}" \${data.apiship_image_class ? `class="\${data.apiship_image_class}"` : ''}>`
					    : '';
					
					  const apishipLink = data.apiship_link
					    ? `<a class="\${data.apiship_link_class || ''}" href="#" onclick="\${data.apiship_link}">
					         \${data.apiship_link_text || ''}
					         <div id="apiship_loading_\${data.code}" class="apiship_loading" style="visibility:hidden;"></div>
					       </a>`
					    : '';
					

					  return `
					    <input type="radio" checked="checked" value="\${data.code}" name="shipping_method">\${apishipImage}\${data.title} - \${data.text} \${apishipLink}
					  `;
					}
					...
					function apiship_open(code) {  
						...
						$.ajax({
							...
							data: {'code':code},
							beforeSend: function() {
								document.getElementById('apiship_loading_' + code).style.visibility = 'visible';
							},
							complete: function() {
								document.getElementById('apiship_loading_' + code).style.visibility = 'hidden';
							},
						...
						$.ajax({
							...
							success: function(data) {								
								...
								{	
									...
									{
										el = create_shipping_method_template(data);
...
EOT;
    ...
    private function fill_template($params)
    {
        ...
        if (isset($params['sub_type'])) $sub_type = $params['sub_type'];
        else $sub_type = '';
        if (isset($params['providerKey'])) $providerKey = $params['providerKey'];
        else $providerKey = '';
        if (isset($params['tariffName'])) $tariffName = sprintf($this->language->get('shipping_apiship_tariff_template'), $params['tariffName']);
        else $tariffName = '';
        if (isset($params['pointName'])) $pointName = $params['pointName'];
        else $pointName = '';
        if (isset($params['pointAddress'])) $pointAddress = $params['pointAddress'];
        else $pointAddress = '';
        if (isset($params['daysMin'])) $daysMin = $params['daysMin'];
        else $daysMin = '';
        if (isset($params['daysMax'])) $daysMax = $params['daysMax'];
        else $daysMax = '';
        if (isset($params['tariffDescription'])) $tariffDescription = $params['tariffDescription'];
        else $tariffDescription = '';
    ...
    public function get_points()
    {
        ...
        if (isset($this->request->post['code'])) $code = $this->request->post['code'];
        else $code = '';
    ...
    public function getQuote($address)
    {
        ...
        if (isset($this->session->data['api_id'])) {
            if (isset($this->request->get['term'])) $search = $this->request->get['term'];
            else $search = "";
    ...
    public function get_label()
    {
        ...
        else return ['error' => $this->apiship_params['shipping_apiship_error_params']];
    ...
    public function export_order()
    {
        ...
        if (isset($this->request->get['id'])) $order_id = $this->request->get['id'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_pickup_type'])) $shipping_apiship_pickup_type = $this->request->get['shipping_apiship_pickup_type'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_place_length'])) $shipping_apiship_place_length = $this->request->get['shipping_apiship_place_length'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_place_width'])) $shipping_apiship_place_width = $this->request->get['shipping_apiship_place_width'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_place_height'])) $shipping_apiship_place_height = $this->request->get['shipping_apiship_place_height'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_place_weight'])) $shipping_apiship_place_weight = $this->request->get['shipping_apiship_place_weight'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_comment'])) $shipping_apiship_comment = $this->request->get['shipping_apiship_comment'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
        if (isset($this->request->get['shipping_apiship_pickup_date'])) $shipping_apiship_pickup_date = $this->request->get['shipping_apiship_pickup_date'];
        else return array('error' => $this->apiship_params['shipping_apiship_error_params']);
    ...
    private function export_order_value(
        $order_id,
        $shipping_apiship_pickup_type,
        $shipping_apiship_place_length,
        $shipping_apiship_place_width,
        ...
        $shipping_apiship_comment,
        $shipping_apiship_pickup_date,
        $shipping_apiship_export_status_ok,
        ...
        if (isset($output_data['orderId'])) {
            ...
            if (isset($output_data['providerNumber'])) $track_number = $output_data['providerNumber'];
            else $track_number = '';
            ...
            if (isset($output_data['message'])) {
                ...
                if (isset($output_data['errors'])) $errors = $output_data['errors'];
                else $errors = [];
                foreach ($errors as $error) {
                ...
                if ($current_status_id != $status_id) {
                    ...
                    if (isset($this->apiship_params['shipping_apiship_mapping_status'][$key])) {
                        if ($this->apiship_params['shipping_apiship_mapping_status'][$key]['use']) {
                            ...
                            if (isset($this->apiship_params['shipping_apiship_mapping_status'][$key]['notify'])) $order_notify = true;
                            else $order_notify = false;
                            $order_text = '';
                        }
                    }
    ...
    public function get_order_params()
    {
        ...
        if (isset($this->request->get['id'])) $order_id = $this->request->get['id'];
        else return array('text' => $this->apiship_params['shipping_apiship_error_params']);
    ...
    private function get_order_params_value($order_id)
    {
        ...
        if (isset($parce_code['provider'])) $provider = $parce_code['provider'];
        else $provider = '';
        if (in_array(2, $this->get_pickup_types($provider))) $apiship_pickup_type = 2;
        else $apiship_pickup_type = 1;
