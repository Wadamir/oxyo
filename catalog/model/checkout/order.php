...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
                // $this->log->write('INSERT ORDER_PRODUCT: product_id=' . $product['product_id'] . ', quantity=' . $product['quantity']);                
    ...
    public function getOrderProducts($order_id) {
        $query = $this->db->query("SELECT op.*, p.sku, p.upc, p.ean, p.jan, p.isbn, p.mpn, p.location, p.image, p.video, p.manufacturer_id FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "product p ON (op.product_id = p.product_id) WHERE op.order_id = '" . (int)$order_id . "'");
