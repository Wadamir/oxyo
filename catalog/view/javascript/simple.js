(function ($) {
        ...
        $block.find('input:checkbox:not(:checked)').each(function () {
            ...
            if (!found) {
                serialized.push({
                    name: this.name,
                    value:
                        this.dataset && this.dataset.uncheckedValue
                            ? this.dataset.uncheckedValue
                            : '',
                });
            }
        });
    ...
    Simple.prototype.getValidationRules = function () {
        ...
        return {
            notEmpty: function ($rule, silent) {
                ...
                if (fieldId) {
                    var $field = $(self.params.mainContainer).find(
                        '#' + fieldId,
                    );

                    if (fieldType == 'checkbox' || fieldType == 'radio') {
                        $field = $(self.params.mainContainer).find(
                            '[id*=' + fieldId + '_]',
                        );
                    }
                ...
                if (fieldId) {
                    ...
                    if ($field.length) {
                        ...
                        if (method) {
                            ...
                            var custom = $rule.attr('data-custom')
                                ? true
                                : false;
                                ...
                                function (data) {
                                    ...
                                    if (data == 'invalid') {
                                        if (!silent) {
                                            ...
                                            if (
                                                self.params.notificationDefault
                                            ) {
                                                $rule.show();
                                            }
                                        }
            ...
            if (fieldId) {
                ...
                if ($field.length) {
                    ...
                    if (
                        ...
                        if ($pairField) {
                            $pairField[$field.attr('data-validate-on')](
                                checker,
                            );
                        }
                    } else {
    ...
    Simple.prototype.checkRules = function (container, silent) {
        ...
        if (
            $container.length &&
            ($container.is(':visible') || $container.find('*').is(':visible'))
        ) {
            $container.find('.simplecheckout-rule-group').each(function () {
                ...
                if (fieldId) {
                    ...
                    if ($field.length) {
                        ...
                        var groupPromise = $.when
                            .apply($, promises)
                            .then(function () {
                                $field.attr(
                                    'data-valid',
                                    ruleGroupResult ? 'true' : 'false',
                                );
    ...
    Simple.prototype.setAddressFields = function (
        block,
        countryId,
        zoneId,
        city,
    ...
    Simple.prototype.initFileUploader = function (
        beforeUploading,
        afterUploading,
    ) {
        var self = this;
                    ...
                    if ($("#form-upload input[name='file']").val() != '') {
                        ...
                        $.ajax({
                            url:
                                'index.php?' +
                                self.params.additionalParams +
                                'route=common/simple_connector/upload',
                            type: 'post',
                            dataType: 'json',
    ...
    Simple.prototype.initAutocomplete = function (callbackAfterChanging) {
        var self = this;
        var $fields = $(self.params.mainContainer).find(
            '#payment_address_city, #shipping_address_city, #register_city, #address_city',
        );
        console.log(
        ...
        var getCheckoutCityContext = function ($field) {
            ...
            var $country = $block
                .find("select[name='country_id'], select[name$='[country_id]']")
                .first();
            var $zone = $block
            ...
            return {
                isCheckoutCityField: true,
                countryId: parseInt($country.val(), 10) || 0,
                zoneId: parseInt($zone.val(), 10) || 0,
            };
        };
                ...
                $field.autocomplete({
                    source: function (request, response) {
                        ...
                        var url =
                            'index.php?' +
                            self.params.additionalParams +
                            'route=common/simple_connector/geo';
                        var data = {
                            term:
                        ...
                        if (cityContext.isCheckoutCityField) {
                            url =
                                'index.php?' +
                                self.params.additionalParams +
                                'route=extension/xd_checkout/checkout/cityAutocomplete';
                            data.country_id = cityContext.countryId;
                            data.zone_id = cityContext.zoneId;
                            data.limit = 12;
                        }
    ...
    Simple.prototype.initTooltips = function (bootstrap) {
        ...
        $mainContainer
            .find('.simplecheckout-tooltip[data-for]')
            .each(function () {
                var $target = $mainContainer.find(
                    '#' + $(this).attr('data-for'),
                );
                ...
                if ($target.length) {
                    if ($target.attr('data-file')) {
                        ...
                        if (
                            bootstrap &&
                            typeof $target.tooltip === 'function'
                        ) {
                            $target.tooltip({
                                html: true,
                                title: $(this).html(),
                            ...
                            $target.easyTooltip({
                                useElement:
                                    ...
                                    " .simplecheckout-tooltip[data-for='" +
                                    $(this).attr('data-for') +
                                    "']",
                                clickRemove: true,
                            });
    ...
    Simple.prototype.initMasks = function () {
        ...
        if (
            typeof $(document).inputmask !==
            'undefined' /*&& !window.localStorage.getItem('inputmaskFailed')*/
        ) {
            var masked = [];
            $mainContainer
                .find('input[data-simple-mask]')
                .each(function (indx) {
            ...
            try {
                for (var i = 0; i < masked.length; i++) {
                    ...
                    $mainContainer.find('input[id=' + id + ']').inputmask({
                        mask: mask,
    ...
    Simple.prototype.initDatepickers = function (callbackAfterChanging) {
        ...
        $(self.params.mainContainer)
            .find(
                'input[type=date],input[type=datetime],input[data-type=date],input[data-type=datetime]',
            )
            .each(function () {
                ...
                if (typeof $(this).datetimepicker !== 'undefined') {
                    ...
                    if (onlyWeekdays || typeof days.length !== 'undefined') {
                        ...
                        for (var i = 0; i < 7; i++) {
                            if (
                                ((i == 6 || i == 0) && onlyWeekdays) ||
                                (useDays && $.inArray(i + '', days) == -1)
                            ) {
                                disabledDays.push(i);
                            }
                        }
                    ...
                    $el.datetimepicker({
                        pickDate: true,
                        pickTime:
                            $el.attr('data-type') == 'datetime' ? true : false,
    ...
    Simple.prototype.initTimepickers = function (callbackAfterChanging) {
        ...
        $(self.params.mainContainer)
            .find('input[type=time],input[data-type=time]')
            .each(function () {
                if (typeof $(this).datetimepicker !== 'undefined') {
                    var $el = $(this);
...
function reloadBlock() {
    if (typeof Simple.prototype.instances !== 'undefined') {
        for (var i in Simple.prototype.instances) {
            ...
            if (
                typeof Simple.prototype.instances[i].reloadBlock === 'function'
            ) {
                Simple.prototype.instances[i].reloadBlock.apply(
                    Simple.prototype.instances[i],
                    arguments,
                );
            }
        }
