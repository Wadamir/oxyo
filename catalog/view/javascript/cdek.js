...
function cdekPvzClick(tariff, tariff_type) {
    ...
    if (typeof cdekymap === 'undefined') {
        console.log('cdekymap is not loaded');
        // alertMessage("Не удалось загрузить карту ПВЗ. Обновите страницу и попробуйте снова.", false);
        return;
    }

    cdekymap.ready(function () {
        initMap(tariff_type);
    });
}
...
function mapShow(status) {
    if (!$('#cdek_map').length) {
        var mapBlock_html =
            '<div class="cdek_map_container" id="cdek_map_contaner" style="dispaly:none">';
        mapBlock_html +=
            '<div class="cdek_map_container_map" id="cdek_map"></div>';
        mapBlock_html +=
            '<div class="cdek_map_container_map_control"><a href="javascript:mapShow(0)" class="control_button">Закрыть</a></div>';
        mapBlock_html += '</div>';
    ...
    $.ajax({
        ...
        success: function (json) {
            if (json.status) {
                ...
                $('#cdek_selectedPvzInfo_' + selected_tariff).html(
                    json.data.address,
                );
            }
        },
