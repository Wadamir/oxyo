$(function () {
    ...
    setFilterButtons(DEFController);
    ...
    $(document).on(
        'click',
        DEFController.filterPanel.selectorButtonClear,
        function () {
            DEFController.reset();
            if ($('html').hasClass('no-scroll side-filter-open')) {
    ...
    if (DEFController.buttonShowMore.isset()) {
        if (!DEFController.url.isFilterParamsInUrl()) {
            DEFController.buttonShowMore.render(
                DEFController.$_GET_string(),
                true,
            );
        }
        ...
        $(document).on(
            'click',
            DEFController.buttonShowMore.selector,
            function () {
...
function setFilterButtons(DEFController) {
    if (
        ...
        console.log('auto apply filter on change');
        $(document).on(
            'change',
            DEFController.filterPanel.selectorControlElements,
            function () {
                DEFController.changeFilterData();
            },
        );
    ...
    if (DEFController.sliderPrice.isset()) {
        ...
        $(DEFController.sliderPrice.selector).slider({
            slide: function (event, ui) {
                DEFController.sliderPrice.setChangePriceMin(
                    $(DEFController.sliderPrice.selector).slider('values', 0),
                );
                DEFController.sliderPrice.setChangePriceMax(
                    $(DEFController.sliderPrice.selector).slider('values', 1),
                );
            },
        });
        ...
        $(document).on(
            'change',
            DEFController.sliderPrice.selectorPriceMin,
            function () {
        ...
        $(document).on(
            'change',
            DEFController.sliderPrice.selectorPriceMax,
            function () {
