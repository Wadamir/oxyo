class ApishipMap {
    constructor() {
        ...
        this.modal = {
            initLayout: {
                template: () => `
					<div class="modal apiship_modal" id="${this.ID_MODAL}" tabindex="-1" role="dialog">
						<div class="modal-dialog modal-fullscreen apiship_modal-dialog" role="document">
        ...
        this.yandexMaps = {
            ...
            initMap: () => {
                ...
                this.Mymap = new ymaps.Map(
                    this.YANDEX_MAP_CONTAINER_ID,
                    {
                        center: [
                            this.yandexMaps.points[0]['lat'],
                            this.yandexMaps.points[0]['lon'],
                        ],
                        zoom: 10,
                        controls:
                            get_yandex_api_key() === ''
                ...
                this.yandexMaps.createPlacemarks(
                    this.yandexMaps.points,
                    this.Mymap,
                );
            },
            createPlacemarks: (points, map) => {
                    ...
                    clusterIconLayout: ymaps.templateLayoutFactory.createClass(
                        '<span class="apiship_cluster"></span>',
                        {
                            build: function () {
                                ...
                                this.getData().properties.geoObjects.forEach(
                                    (geoObject) => {
                                        if (providerKey === '') {
                                            providerKey =
                                            ...
                                            providerKey !==
                                            geoObject.properties.providerKey
                                        ) {
                                        ...
                                        if (cost_min === 0) {
                                            cost_min =
                                                geoObject.properties.cost;
                                            text_min =
                                                geoObject.properties.text;
                                        } else if (
                                            geoObject.properties.cost < cost_min
                                        ) {
                                            cost_min =
                                                geoObject.properties.cost;
                                            text_min =
                                ...
                                if (changeProviderKey) {
                                    ...
                                    el.innerHTML =
                                        '<img style="width:64px;vertical-align: middle;" src="https://storage.apiship.ru/icons/providers/svg/' +
                                        providerKey +
                                        '.svg">' +
                                        ' от ' +
                                        text_min;
                ...
                for (const point of points) {
                    if (!point_types.includes(point.type))
                        point_types.push(point.type);
                    if (!providers.includes(point.provider))
                        providers.push(point.provider);

                    const balloonContentBody =
                        '<h3 style="font-size: 1.3em;font-weight: bold;margin-bottom: 0.5em;">' +
                        point.address +
                        '</h3>' +
                        '<b>Стоимость: </b>' +
                        point.text +
                        '<br>' +
                        (point.paymentCash == 1
                            ? '<img title="Оплата наличными" src="./catalog/view/theme/default/image/shipping/apiship_cash.png">'
                            : '') +
                        ' ' +
                        (point.paymentCard == 1
                            ? '<img title="Оплата картой" src="./catalog/view/theme/default/image/shipping/apiship_card.png">'
                            : '');
                    ...
                    objectManager.add({
                        ...
                        properties: {
                            ...
                            balloonContentFooter:
                                '<a href=# data-placemarkid="' +
                                point.code +
                                '" class="list_item btn btn-success">Забрать отсюда</a>',
                        },
                        options: {
                            ...
                            iconContentLayout:
                                ymaps.templateLayoutFactory.createClass(
                                    '<span class="apiship_cluster"><img style="width:64px;vertical-align: middle;" src="https://storage.apiship.ru/icons/providers/svg/' +
                                        point.provider_key +
                                        '.svg"> ' +
                                        point.text +
                                        '</span>',
                ...
                let listBoxControlTypes = new ymaps.control.ListBox({
                    ...
                    state: {
                        filters: pointTypesItems.reduce((filters, filter) => {
                            filters[filter.data.get('content')] =
                                filter.isSelected();
                            return filters;
                        }, {}),
                    },
                ...
                let listBoxControlProviders = new ymaps.control.ListBox({
                    ...
                    state: {
                        filters: pointProvidersItems.reduce(
                            (filters, filter) => {
                                filters[filter.data.get('content')] =
                                    filter.isSelected();
                                return filters;
                ...
                listBoxControlTypes.events.add(['select', 'deselect'], (e) => {
                    ...
                    let filters = ymaps.util.extend(
                        {},
                        listBoxControlTypes.state.get('filters'),
                    );
                    filters[listBoxItem.data.get('content')] =
                        listBoxItem.isSelected();
                    listBoxControlTypes.state.set('filters', filters);
                });

                listBoxControlProviders.events.add(
                    ['select', 'deselect'],
                    (e) => {
                        let listBoxItem = e.get('target');
                        let filters = ymaps.util.extend(
                            {},
                            listBoxControlProviders.state.get('filters'),
                        );
                        filters[listBoxItem.data.get('content')] =
                            listBoxItem.isSelected();
                ...
                // Мониторинг фильтров
                let filterMonitorTypes = new ymaps.Monitor(
                    listBoxControlTypes.state,
                );
                ...
                let filterMonitorProviders = new ymaps.Monitor(
                    listBoxControlProviders.state,
                );
                ...
                $(document).on('click', 'a.list_item', (event) => {
                    ...
                    this.callback_function(
                        $(event.currentTarget).data().placemarkid,
                        this.callback_code,
                    );
                    this.onCloseModal();
                    this.modal.close();
