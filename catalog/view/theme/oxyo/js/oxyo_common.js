...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.dropdown_mobile h5').forEach(function (title) {
        title.addEventListener('click', function () {
            let dropdownMobile = this.parentElement;
            dropdownMobile.classList.toggle('opened');
        });
    });
});
...
const quickview = function (product_id) {
    ...
    fetch('index.php?route=extension/oxyo/quickview&product_id=' + product_id, {
        ...
        .then((html) => {
            ...
            const tooltips = contentEl.querySelectorAll(
            ...
            if (window.$ && $.fn.slick) {
                ...
                setTimeout(() => {
                    sliderEl.slick({
                        prevArrow:
                            '<a class="arrow-left within icon-arrow-left"></a>',
                        nextArrow:
                            '<a class="arrow-right within icon-arrow-right"></a>',
                        arrows: true,
                    });
                }, 100);
            } else {
...
function addToCartQuickview() {
    ...
    fetch('index.php?route=extension/oxyo/oxyo_features/add_to_cart', {
        ...
        .then((json) => {
            document
                .querySelectorAll('.alert, .text-danger, .popup-note')
                .forEach((e) => e.remove());
            document
                .querySelectorAll('.table-cell, .has-error')
            ...
            if (json.error) {
                ...
                if (json.error.recurring) {
                    const recurringSelect = form.querySelector(
                    ...
                    if (recurringSelect) {
                        ...
                        recurringSelect.insertAdjacentElement(
                            'afterend',
                            recurringError,
                        );
                    }
                ...
                if (cartContent) {
                    fetch('index.php?route=common/cart/info')
                        ...
                        .then((html) => {
                            ...
                            temp.innerHTML = html;
                            const newContent =
                                temp.querySelector('#cart-content');
                            if (newContent) {
...
var subscribe = function (module) {
    $.ajax({
        url:
            'index.php?route=extension/oxyo/oxyo_features/subscribe&module=' +
            module,
        type: 'post',
        dataType: 'json',
        data:
...
var unsubscribe = function (module) {
    $.ajax({
        url:
            'index.php?route=extension/oxyo/oxyo_features/unsubscribe&module=' +
            module,
        type: 'post',
        dataType: 'json',
        data:
    ...
    add: function (product_id, quantity, source) {
        $.ajax({
            ...
            beforeSend: function (json) {
                $('body').append(
                    '<span class="oxyo-spinner ajax-call"></span>',
                );
            },
            ...
            success: function (json) {
                ...
                if (json['success_redirect']) {
                    ...
                    $(
                        '.alert, .popup-note, .oxyo-spinner.ajax-call, .text-danger',
                    ).remove();
    ...
    $.ajax({
        ...
        complete: function () {
            $('#mail_form' + form_id + ' .captchaimg').attr(
                'src',
                'index.php?route=extension/oxyo/oxyo_features/oxyo_captcha#' +
                    new Date().getTime(),
            );
