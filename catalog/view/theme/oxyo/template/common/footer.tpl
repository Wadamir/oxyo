    ...
    <div class="container">
        ...
        <div class="row links-holder">
            <div class="col-xs-12 col-sm-8">
                <div class="row">
                    <?php if ($custom_links) { ?>
                        <?php foreach ($oxyo_footer_columns as $column) { ?>
                            <div class="footer-column col-xs-12 col-sm-6 <?php echo $oxyo_columns_count; ?> eq_height">
                                <?php if ($column['title']) { ?>
                                    <h5><?php echo $column['title']; ?></h5>
                                <?php } ?>
                                <?php if (isset($column['links'])) { ?>
                                    <?php usort($column['links'], function ($a, $b) {
        ...
        <?php if (isset($oxyo_copyright)) { ?>
            <div class="footer-copyright"><?php echo $oxyo_copyright; ?></div>
        <?php } ?>
    </div>
</div>
    ...
    <script>
        if ($(window).width() > 767) {
            $('#column-left, #column-right').theiaStickySidebar({
                ...
                additionalMarginTop: <?php echo $sticky_columns_offset; ?>
            });
        }
    </script>
<?php } ?>
<?php if ($view_cookie_bar) { ?>
    ...
    <div class="oxyo_cookie_bar">
        <div class="table">
            <div class="table-cell w100"><?php echo $oxyo_cookie_info; ?></div>
            <div class="table-cell button-cell">
                <a class="btn btn-tiny btn-light-outline" onclick="$(this).parent().parent().parent().fadeOut(400);"><?php echo $oxyo_cookie_btn_close; ?></a>
                <?php if (!empty($href_more_info)) { ?>
                    <a class="more-info anim-underline light" href="<?php echo $href_more_info; ?>"><?php echo $oxyo_cookie_btn_more_info; ?></a>
                <?php } ?>
            </div>
        </div>
    </div>
...
OXYO VERSION <?php echo $oxyo_version; ?> - OPENCART VERSION 2.3 (<?php echo VERSION; ?>)
//-->
