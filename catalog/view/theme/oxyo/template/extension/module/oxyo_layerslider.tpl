                ...
                <div class="ms-slide <?php if (!empty($section['slide_kenburn'])) echo 'kenburn ' . $section['slide_kenburn']; ?>" data-delay="<?php echo $section['duration']; ?>" <?php if (!empty($section['bg_color'])) { ?>style="background-color:<?php echo $section['bg_color']; ?>" <?php } ?>>
                    ...
                    <?php foreach ($section['groups'] as $key => $group) { ?>
                        ...
                        <?php if ($group['type'] == 'button') { ?>
                            <!-- Button layer -->
                            <a class="ms-layer ms-btn <?php echo $group['button_class']; ?> <?php if ($group['button_target']) {
                                                                                                echo 'external';
                                                                                            } ?>" href="<?php echo $group['button_href']; ?>" style="white-space:nowrap;"
...
<script>
    $(document).ready(function() {
        ...
        slider.setup('masterslider<?php echo $module; ?>', {
            ...
            overPause: false,
            <?php if ($loop) { ?>loop: true,
        <?php } ?>
        speed: <?php echo $speed; ?>,
        view: "<?php echo $slide_transition; ?>"
        });
        <?php if ($nav_buttons) { ?>slider.control('arrows', {
            autohide: false
        });
    <?php } ?>
    <?php if ($nav_bullets) { ?>slider.control('bullets', {
        autohide: false
    });
    <?php } ?>
    <?php if ($nav_timer_bar) { ?>slider.control('timebar', {
        width: 5
    });
    <?php } ?>
