...
/**
 * Helper to build responsive display classes for footer
 * based on *_footer_xs, *_footer_md, *_footer_lg, *_footer_xl flags.
 *
 * Logic:
 ...
 */
$footer_breakpoints = array('xs', 'md', 'lg', 'xl');
...
foreach ($contacts_config as $name => $cfg) {
    ...
    // Read footer_* breakpoint flags and build responsive classes
    $bp_values = array();
    foreach ($footer_breakpoints as $bp) {
        // config key: main_phone_footer_xs, whatsapp_footer_md, etc.
        $key = $name . '_footer_' . $bp;
    ...
    // Common wrapper class for footer icon element
    $wrapper_base_class = 'icon-element menu-element';
    $footer_display_class = $wrapper_base_class . ' ' . implode(' ', $display_classes);

    if ($name === 'callback') {
        // Additional class for callback to identify it in JS
        $footer_display_class .= ' callback_action d-none d-md-flex animate-text animate-splash';
    }
    ...
    $data[$name . '_footer_class']  = $footer_display_class;
    ...
    $contact_array[] = array(
        ...
        'link'           => $link,
        'onclick'        => $onclick,
        'footer_class'   => $footer_display_class,
        'footer_flags'   => $bp_values, // если хочешь использовать в Twig по-отдельности
    );
}
