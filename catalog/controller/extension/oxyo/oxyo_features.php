<?php
class ControllerExtensionOxyoOxyoFeatures extends Controller
{
    ...
    public function add_to_cart()
    {
        ...
        if ($product_info) {
            ...
            if (!$json) {
                ...
                if ($this->config->get('oxyo_cart_action') == 'redirect_cart') $json['success_redirect'] = $this->url->link('checkout/cart');
    ...
    public function add_to_compare()
    {
        ...
        if ($product_info) {
            ...
            if ($this->config->get('oxyo_compare_action') == 'redirect') $json['success_redirect'] = $this->url->link('product/compare');
    ...
    public function add_to_wishlist()
    {
        ...
        if ($product_info) {
            ...
            if ($this->config->get('oxyo_wishlist_action') == 'redirect') $json['success_redirect'] = $this->url->link('account/wishlist');
    ...
    // Search
    public function oxyo_search()
    {

        $this->load->language('oxyo/oxyo_theme');

        $data['oxyo_text_search'] = $this->language->get('oxyo_text_search');
        $data['oxyo_text_category'] = $this->language->get('oxyo_text_category');
        ...
        if ($this->config->get('theme_default_directory') == 'oxyo') {
            if ($this->config->get('oxyo_header') == 'header6') {
                return $this->load->view('common/oxyo_search_full', $data);
            } else {
                return $this->load->view('common/oxyo_search', $data);
            }
        }
    }
    ...
    public function subscribe()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            if (!filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
                $json['error'] = $this->language->get('oxyo_subscribe_invalid_email');
            }
            if (!isset($json['error'])) {
                $this->load->model('extension/oxyo/newsletter');
                if ($this->model_extension_oxyo_newsletter->checkRegistered($this->request->post)) {
                    $this->model_extension_oxyo_newsletter->UpdateRegistered($this->request->post, 1);
                    $json['success'] = $this->language->get('oxyo_subscribe_success');
                } else if ($this->model_extension_oxyo_newsletter->checkExist($this->request->post)) {
                    $json['error'] = $this->language->get('oxyo_subscribe_email_exist');
                } else {
                    $this->model_extension_oxyo_newsletter->subscribe($this->request->post);
                    $json['success'] = $this->language->get('oxyo_subscribe_success');
                }
            }
        }
    ...
    public function unsubscribe()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if (!isset($json['error'])) {
                $this->load->model('extension/oxyo/newsletter');
                if ($this->model_extension_oxyo_newsletter->checkRegistered($this->request->post)) {
                    $this->model_extension_oxyo_newsletter->UpdateRegistered($this->request->post, 0);
                    $json['success'] = $this->language->get('oxyo_unsubscribe_unsubscribed');
                } else if (!$this->model_extension_oxyo_newsletter->checkExist($this->request->post)) {
                    $json['error'] = $this->language->get('oxyo_unsubscribe_not_found');
                } else {
                    $this->model_extension_oxyo_newsletter->unsubscribe($this->request->post);
                    $json['success'] = $this->language->get('oxyo_unsubscribe_unsubscribed');
                }
            }
        }
    ...
    public function oxyo_send_message()
    {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if (!isset($json['error'])) {
                ...
                $mail->setSubject(html_entity_decode(sprintf($this->language->get('oxyo_email_subject'), $this->request->post['name']), ENT_QUOTES, 'UTF-8'));
                $mail->setText($this->request->post['text']);
                $mail->send();
                $json['success'] = $this->language->get('oxyo_text_success_form');
            }
        }
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json));
    ...
    public function oxyo_popup()
    {
        ...
        $data['popup_width'] = $this->config->get('oxyo_popup_note_w');
        $data['popup_height'] = $this->config->get('oxyo_popup_note_h');
        ...
        if (is_file(DIR_IMAGE . $this->config->get('oxyo_popup_note_img'))) {
            $data['img'] = $server . 'image/' . $this->config->get('oxyo_popup_note_img');
        } else {
            $data['img'] = '';
        }

        $content_block = $this->config->get('oxyo_popup_note_block');
        ...
        $content_title = $this->config->get('oxyo_popup_note_title');
