...
class ControllerExtensionModuleHpmodelHpmodel extends Controller
{
    ...
    public function getForm()
    {
        ...
        $type['setting']['main_title'] = !empty($type['setting']['product_group_title'][$language_id]) ? html_entity_decode($type['setting']['product_group_title'][$language_id], ENT_QUOTES, 'UTF-8') : '';
        $type['setting']['title'] = !empty($type['setting']['product_title'][$language_id]) ? html_entity_decode($type['setting']['product_title'][$language_id], ENT_QUOTES, 'UTF-8') : '';
        ...
        uasort($data['groups'], function ($a, $b) {
            return (int)$a['value']['sort_order'] <=> (int)$b['value']['sort_order'];
        });
        ...
        $data['selector'] = !empty($type['setting']['product_selector']) ? $type['setting']['product_selector'] : false;
        $data['position'] = !empty($type['setting']['product_position']) ? $type['setting']['product_position'] : false;
        $data['title'] = $type['setting']['main_title'];
    ...
    private function prepareProducts($setting, $childs, $parent)
    {
        ...
        foreach ($childs as $child) {
            ...
            if (!empty($child['image'])) $product_images[$child['product_id']] = $child['image'];
        }
        ...
        foreach ($products_data as $product_info) {
            ...
            if (!empty($product_images[$product_id])) {
                $product_info['image'] = $product_images[$product_id];
            }
            ...
            foreach ($config_groups as $group_id => &$config_group) {
                ...
                if ($column == 'attribute') {
                    ...
                    $key_value = (float)$product_info['weight'];
                } else if ($column == 'col_size') {
                    $key_value = (float)$product_info['length'] . 'x' . (float)$product_info['width'] . 'x' . (float)$product_info['height'];
                } else if ($column == 'name_exp_last') {
                    $name_arr = explode(' ', $product_info['name']);
                    $key_value = array_pop($name_arr);
                } else {
                ...
                foreach ($config_group['cols'] as $key => $col) {
                    ...
                    switch ($col['type']) {
                        ...
                        case 'filter':
                            ...
                            break;
                        case 'col_weight':
                            $value = (float)$product_info['weight'] . ' ' . $this->weight->getUnit($product_info['weight_class_id']);
                            $value_text = $value;
                            break;
                        case 'col_size':
            ...
            $product_info['hpm_tv'] = trim(str_replace(["'", "\r", "\n"], ["\\'", "", ""], implode(', ', $tv_text)));

            if (!empty($setting['redirect'])) {
                $product_info['href'] = $this->url->link('product/product', (!empty($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') . 'product_id=' . $parent['parent_id']) . '#' . implode('-', $keys);
            } else {
                $product_info['href'] = $this->url->link('product/product', (!empty($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') . 'product_id=' . $product_info['product_id']);
            }
            $product_info['href'] = html_entity_decode($product_info['href'], ENT_QUOTES, 'UTF-8');
            $product_info['href_real'] = $this->url->link('product/product', (!empty($this->request->get['path']) ? 'path=' . $this->request->get['path'] . '&' : '') . 'product_id=' . $product_info['product_id']);
            $product_info['href_real'] = html_entity_decode($product_info['href_real'], ENT_QUOTES, 'UTF-8');

            $product_info['price'] = str_replace('"', '\'', $this->currency->format($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']));
            $product_info['special'] = (float)$product_info['special'] ? str_replace('"', '\'', $this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'])) : false;
    ...
    public function getCategoryBlock($product_info)
    {
        ...
        if (!isset($product_info['product_id']) || !isset($product_info['quantity'])) return $product_info;
        ...
        $type['setting']['product_filters'] = !empty($type['setting']['category_filters']) ? $type['setting']['category_filters'] : array();
        ...
        $type['setting']['main_title'] = !empty($type['setting']['category_group_title'][$language_id]) ? html_entity_decode($type['setting']['category_group_title'][$language_id], ENT_QUOTES, 'UTF-8') : '';
        $type['setting']['title'] = !empty($type['setting']['category_title'][$language_id]) ? html_entity_decode($type['setting']['category_title'][$language_id], ENT_QUOTES, 'UTF-8') : '';
        ...
        $type['setting']['after_title_filter'] = !empty($type['setting']['category_after_title_filter']) ? $type['setting']['category_after_title_filter'] : false;
        ...
        */
        ...
        foreach ($data['groups'] as &$group) {
            ...
            foreach ($group['value'] as &$value) {
                $value['active'] = in_array($product_info['product_id'], $value['id']);
            }
        }
