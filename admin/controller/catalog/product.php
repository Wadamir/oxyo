    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            ...
            $data['products'][] = array(
                ...
                'name'       => $result['name'],
                'model'      => $result['model'],
                'attributes' => $product_attributes_array,
    ...
    protected function getForm()
    {
        ...
        if (isset($this->request->post['video']) && is_file(DIR_IMAGE . $this->request->post['video'])) {
            if ($this->request->server['HTTPS']) {
                $data['video_thumb'] = HTTPS_CATALOG . 'image/' . $this->request->post['video'];
            } else {
                $data['video_thumb'] = HTTP_CATALOG . 'image/' . $this->request->post['video'];
            }
            $data['video_extension'] = pathinfo($this->request->post['video'], PATHINFO_EXTENSION);
            $data['video_name'] = pathinfo($this->request->post['video'], PATHINFO_FILENAME);
        } elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['video'])) {
            if ($this->request->server['HTTPS']) {
                $data['video_thumb'] = HTTPS_CATALOG . 'image/' . $product_info['video'];
            } else {
                $data['video_thumb'] = HTTP_CATALOG . 'image/' . $product_info['video'];
            }
            $data['video_extension'] = pathinfo($product_info['video'], PATHINFO_EXTENSION);
            $data['video_name'] = pathinfo($product_info['video'], PATHINFO_FILENAME);
        } else {
            $data['video_thumb'] = '';
            $data['video_extension'] = '';
            $data['video_name'] = '';
        }
