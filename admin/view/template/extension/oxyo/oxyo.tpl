...
<div id="content">
  <div class="page-header">
	...
	<button type="submit" form="form-oxyo" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="btn btn-primary">Save</button>
  ...
  <div class="container-fluid">    
   ...
   <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-oxyo" class="form-horizontal">
...
$('select[name=\'store_id\']').on('change', function () {
    location = 'index.php?route=extension/oxyo/oxyo&token=<?php echo $token; ?>&store_id=' +  encodeURIComponent($(this).val());
});
//--></script>
...
if ($(this).val() == '1') {
	$('#oxyo_typo_holder').css('display', 'block');
} else {
	$('#oxyo_typo_holder').css('display', 'none');
}
});
